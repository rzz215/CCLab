...
function setup() {
  let canvas= createCanvas(800, 500); canvas.parent("p5-canvas-container");
...
function draw() {
  ...
  drawFlower(130, 340, 20, color(105, 150, 170));
  ...
  drawFlower(330, 470, 11, color(143, 126, 180));
  drawFlower(520, 380, 14, color(77, 156, 75));
  drawFlower(560, 440, 13, color(116, 86, 140));
  drawFlower(365, 410, 19, color(116, 186, 140));
  ...
  for (let i = 10; i < width; i += 30) {
   ...
   // Bushes
  let circleSize = sin(sinInput1)*5;
    
  
  noStroke();
  fill(0);
  circle(20, 500, 220 +circleSize);
  circle(100, 500, 220+circleSize);
  circle(0, 420, 220+circleSize);
  circle(170, 530, 220+circleSize);
  circle(690, 500, 220+circleSize);
  circle(800, 420, 220+circleSize);
  circle(750, 530, 220+circleSize);
  sinInput1= sinInput1 + speed1;
  ...
  if (isHappy) {
  background(222, 219, 129, 10); //happy
} else {
  background(7, 20, 135, 50); // unhappy
}
...
function drawFlower(x, y, colorP, flowerColor) {
  ...
  let flowerHovered = color(random(255), random(255), random(255), 50);

  if (distToMouse < 50) {
    flowerColor = flowerHovered;
  }
  fill(flowerColor);
  noStroke();
  // petals
